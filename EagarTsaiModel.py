...
class EagarTsai():
    ...
    def edgegraft(self, sol, phi, orientation):
        ...
        l_x_new = int(self.location[0]/self.dimstep + l*np.cos(phi)/self.dimstep)
        l_y_new = int(self.location[1]/self.dimstep + l*np.sin(phi)/self.dimstep)
      ...
      #  breakpoint()
       ...
       # breakpoint()
       # print("UPDATE_x",int(l*np.cos(phi)/self.dimstep) )
       # print("UPDATE_x",int(np.rint(l*np.cos(phi)/self.dimstep)) )
        if self.location_idx[0] + int(np.rint(l*np.cos(phi)/self.dimstep)) < 0:
            breakpoint()
        self.location_idx[0] = np.argmin(np.abs(self.location[0] - self.xs))#+= int(np.rint(l*np.cos(phi)/self.dimstep))
        self.location_idx[1] =  np.argmin(np.abs(self.location[1] - self.ys))#+= int(np.rint(l*np.sin(phi)/self.dimstep))

        self.visitedx.append(self.location_idx[0])
        self.visitedy.append(self.location_idx[1])
        if -1 in self.visitedx:
            breakpoint()
  ...
  #      check if boundary condition is needed:
    ...
    def check(self, dt, phi, V):
        ...
        corner, edge, ddim, edges, distprime, ellipse =  _checkellipse(rxf, rxr, ry, rz, l_x, l_y, l_idx, l_idy, ellipse, self.xs, self.ys, phi, np.round(self.location[0], decimals = 10),  np.round(self.location[1], decimals = 10))
    ...
    def plot(self):
        ...
        pcm0 = axes[0].pcolormesh(self.xs, self.ys, self.theta[:, :, -1].T, shading = 'gouraud', cmap = 'jet', vmin = 300, vmax = 1673)
       # axes[0].plot(self.location[0], self.location[1] ,'r.')
      #  axes[0].plot(self.xs[self.location_idx[0]], self.ys[self.location_idx[1]], 'k.')
        #breakpoint()
        pcm1 = axes[1].pcolormesh(self.xs, self.zs, self.theta[:, len(self.ys)//2, :].T, shading = 'gouraud', cmap = 'jet', vmin = 300, vmax = 1673)
        pcm2 = axes[2].pcolormesh(self.ys, self.zs, self.theta[xcurrent, :, :].T, shading = 'gouraud', cmap = 'jet', vmin = 300, vmax = 1673)
        pcms = [pcm0, pcm1, pcm2]
        scale_x = 1e-6
        scale_y = 1e-6
        ticks_x = ticker.FuncFormatter(lambda x, pos: '{0:g}'.format(x/scale_x))
        ...
        for axis, pcm, fig in zip(axes, pcms, figures):
            ...
            axis.set_title(str(round(self.time*1e6)) + r'[$\mu$s] ' + "Power: " + str(int(np.around(self.P))) + "W" + " Velocity: " + str(np.around(self.V, decimals = 2)) + r" [m/s]")
    ...
    def diffuse(self, dt):
        ...
        # pcm0 = plt.pcolormesh(self.xs, self.ys, self.theta[:, :, -1].T, shading = 'gouraud', cmap = 'jet', vmin = 300, vmax = 1673)
        # plt.gca().set_aspect('equal')
        # plt.pause(0.1)
        ...
        for j in range(len(self.ys)):
            for i in range(len(self.xs)):               
                if self.theta[i, j, -1] > 1673:
                    ...
                    self.contour[i, j] = 180e-6+ root
        
        # from mpl_toolkits import mplot3d
        # ax4 = plt.axes(projection = '3d')

        # ax4.plot_surface(self.xs, self.ys, self.contour,  cmap = 'binary')
