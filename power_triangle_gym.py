...
class EnvRLAM(gym.Env):
    ...
    def __init__(self, plot=False, frameskip=2, verbose=0):
    ...
    def step(self, action):
        ...
        for m in range(self.frameskip):
          ...
          #  breakpoint()
            ...
            if self.plot:
                np.savetxt(
                    fig_dir + "/" + "powercontroltriangletimesnorm", np.array(self.times)*1e3)
                np.savetxt(fig_dir + "/" + "powercontroltriangledepthsframeskip" +
                           str(self.frameskip), np.array(self.depths))
                np.savetxt(fig_dir + "/" + "powercontroltrianglevelocityframeskip" +
                           str(self.frameskip), np.array(self.velocity))
                np.savetxt(fig_dir + "/" + "powercontroltrianglepowerframeskip" +
                           str(self.frameskip), np.array(self.power))
                testfigs = self.ETenv.plot()
                testfigs[0].savefig(fig_dir + "/" + str(self.frameskip) +
                                    'powercontroltriangle_test' + '%04d' % self.timesteps + ".png")
                plt.clf()
                ...
                plt.savefig(fig_dir + "/" + str(self.frameskip) +
                            'powercontroltriangletestdepth' + '%04d' % self.timesteps + ".png")
                plt.clf()
                ...
                plt.title(str(round(self.ETenv.time*1e6)) + r'[$\mu$s] ')
                plt.savefig(fig_dir + "/" + str(self.frameskip) +
                            'powercontroltriangletestvelocity' + '%04d' % self.timesteps + ".png")
                plt.clf()
                plt.close('all')
                ...
                plt.title(str(round(self.ETenv.time*1e6)) + r'[$\mu$s] ')
                plt.savefig(fig_dir + "/" + str(self.frameskip) +
                            'powercontroltriangletestpower' + '%04d' % self.timesteps + ".png")
                plt.clf()
                plt.close('all')
